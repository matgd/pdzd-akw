...
echo "[$(date)] ################### STARTING SCRIPT ###################" | tee -a "${LOGFILE}"
echo "[$(date)] INFO: Trying to fetch data from kaggle..." | tee -a "${LOGFILE}"
...
function downloadSet() {
    ...
    for i in $(seq ${RETRIES}); do
        echo "[$(date)] INFO: Attempt ${i}/${RETRIES} of fetching dataset ${1}..." | tee -a "${LOGFILE}"
        printf "[$(date)] KAGGLE: " >> ${LOGFILE}
        /home/cloudera/.local/bin/kaggle datasets download "${1}" -p "${2}" | tee -a "${LOGFILE}"
        if [[ ${PIPESTATUS[0]} -eq 0 ]]; then
            return 0
        fi
        echo "[$(date)] INFO: Attempt ${i} failed, retry in ${SLEEP_TIME} seconds..." | tee -a "${LOGFILE}"
        sleep ${SLEEP_TIME}
    done
    echo "[$(date)] ERROR: Failed fetching dataset ${1}" | tee -a "${LOGFILE}"
    return 1
}

echo "[$(date)] INFO: Finished download." | tee -a "${LOGFILE}"
...
if [[ "${EXIT_CODE}" -ne 0 ]]; then
    echo "[$(date)] ERROR: Failed fetching at least one dataset (code: ${EXIT_CODE})" | tee -a "${LOGFILE}"
    exit "${EXIT_CODE}"
fi
...
if [[ -f "${DATASET_CSV_1}.old" ]]; then
    ...
    old_lines_1=$(wc -l < ${DATASET_CSV_1}.old)
...
if [[ -f "${DATASET_CSV_2}.old" ]]; then
    ...
    old_lines_2=$(wc -l < ${DATASET_CSV_2}.old)
    new_lines_2=$(wc -l < ${DATASET_CSV_2})
...
echo "[$(date)] INFO: Marking files as old ones in ${TARGET_DIR_1}..." | tee -a "${LOGFILE}"
for f in "${TARGET_DIR_1}/"*.csv; do
    cp "${f}" "${f}.old"
done

echo "[$(date)] INFO: Marking files as old ones in ${TARGET_DIR_2}..." | tee -a "${LOGFILE}"
